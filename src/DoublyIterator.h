...
template <class T> class DoublyLinkedList;	//교차헤더 포함을 피하기위한 명시적 선언
...
{
	friend class DoublyLinkedList<T>;	//private변수 접근을 위한 프렌드선언
public:
	...
	*	@return	this포인터가 반환된다.
	*/
	DoublyIterator<T>& operator=(DoublyLinkedList<T>& list);
private:
	///사용할 리스트의 참조 변수
...
//대입연산자 오버로딩
template<class T>
inline DoublyIterator<T>& DoublyIterator<T>::operator=(DoublyLinkedList<T>& list)
{
